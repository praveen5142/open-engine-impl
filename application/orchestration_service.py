...
class OrchestrationService:
    ...
    def _auto_resolve_payload(self, task_id: int, role: Role, payload: dict) -> dict:
        ...
        pipeline) didn't supply it explicitly, so the caller never has to
        manually pass a work_order between legs.
        """
        payload = dict(payload or {})
        with closing(sqlite3.connect(self.db_path)) as conn:
            if role == Role.EXECUTION and not payload.get("work_order"):
                row = conn.execute(
    ...
    def next_role(self, task_id: int) -> Optional[Role]:
        """
        Decide what the autonomous pipeline should run next for this task, so
        a single 'Delegate Routing' click can chain PLANNING -> EXECUTION ->
        REVIEW to completion without the caller picking a role. Returns None
        once the task is done (REVIEW approved) or a review-retry loop has
        exhausted its budget and now needs a human.
        """
        with closing(sqlite3.connect(self.db_path)) as conn:
            runs = conn.execute(
